...
export function SkillsModal(props: { view: SkillView; setView: (view: SkillView) => void; scope: string; items: Array<PageSkillSummary | SkillCatalogItem>; onClose: () => void; onRefresh: () => void; onUse: (skill: Pick<SkillCatalogItem, "name" | "description">, debug?: boolean) => void; onInstall: (slug: string, update: boolean) => void; onToggle: (slug: string, enabled: boolean) => void; onEdit: (slug: string) => void }) {
  const { t } = useTranslation();
  return <ModalShell title={t("skills.title")} subtitle={t("skills.subtitle")} onClose={props.onClose} action={<IconButton label={t("skills.refresh")} onClick={props.onRefresh}><RefreshCw size={15} /></IconButton>}><div className="grid grid-cols-3 rounded-xl bg-slate-100 p-1">{(["page", "installed", "marketplace"] as const).map((view) => <button key={view} onClick={() => props.setView(view)} className={`rounded-lg px-2 py-2 text-[11px] font-medium ${props.view === view ? "bg-white text-slate-900 shadow-sm" : "text-slate-500"}`}>{view === "page" ? t("skills.currentPage") : view === "installed" ? t("skills.mine") : t("skills.market")}</button>)}</div><p className="px-1 pt-3 text-[10px] text-slate-400">{props.view === "page" ? props.scope : t("skills.count", { count: props.items.length })}</p><div className="mt-2 space-y-2">{props.items.length ? props.items.map((skill) => <SkillRow key={skill.slug} skill={skill} view={props.view} onUse={props.onUse} onInstall={props.onInstall} onToggle={props.onToggle} onEdit={props.onEdit} />) : <p className="rounded-2xl border border-dashed border-slate-200 px-4 py-8 text-center text-xs text-slate-400">{t("skills.empty")}</p>}</div></ModalShell>;
}
...
function SkillRow({ skill, view, onUse, onInstall, onToggle, onEdit }: { skill: PageSkillSummary | SkillCatalogItem; view: SkillView; onUse: (skill: Pick<SkillCatalogItem, "name" | "description">, debug?: boolean) => void; onInstall: (slug: string, update: boolean) => void; onToggle: (slug: string, enabled: boolean) => void; onEdit: (slug: string) => void }) {
  ...
  return <article className={`rounded-2xl border border-slate-200 bg-white p-3 ${pageSkill && !pageSkill.enabled ? "opacity-55" : ""}`}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><strong className="block truncate text-xs">{skill.name}</strong><span className="mt-1 inline-block rounded-full bg-violet-50 px-2 py-0.5 text-[9px] text-violet-600">{t(`skills.scope.${skill.scope}`)}</span></div>{view === "marketplace" && catalogSkill ? <button disabled={catalogSkill.installed && !catalogSkill.updateAvailable} onClick={() => onInstall(skill.slug, catalogSkill.updateAvailable)} className="rounded-lg bg-slate-950 px-2.5 py-1.5 text-[10px] font-medium text-white disabled:bg-slate-100 disabled:text-slate-400">{catalogSkill.updateAvailable ? t("action.update") : catalogSkill.installed ? t("action.installed") : t("action.install")}</button> : <button disabled={Boolean(pageSkill && !pageSkill.enabled)} onClick={() => onUse(skill)} className="rounded-lg bg-slate-950 px-2.5 py-1.5 text-[10px] font-medium text-white disabled:bg-slate-200">{t("action.use")}</button>}</div><p className="mt-2 text-[11px] leading-4 text-slate-500">{skill.description}</p>{view !== "marketplace" ? <div className="mt-2 flex gap-3 text-[10px] text-slate-400"><button onClick={() => onUse(skill, true)}>{t("action.debug")}</button>{pageSkill?.configurable ? <><button onClick={() => onToggle(skill.slug, !pageSkill.enabled)}>{pageSkill.enabled ? t("action.disable") : t("action.enable")}</button><button onClick={() => onEdit(skill.slug)}>{t("action.edit")}</button></> : catalogSkill?.stepCount ? <button onClick={() => onEdit(skill.slug)}>{t("action.edit")}</button> : null}</div> : null}</article>;
}
...
export function RecordingModal(props: { active: boolean; actions: RecordedBrowserAction[]; name: string; description: string; editing: boolean; onName: (value: string) => void; onDescription: (value: string) => void; onClose: () => void; onToggle: () => void; onReplay: () => void; onSave: () => void }) {
  const { t } = useTranslation();
  return <ModalShell title={props.active ? t("recording.activeTitle") : props.editing ? t("recording.editTitle") : t("recording.completedTitle")} subtitle={t("recording.capturedSteps", { count: props.actions.length })} onClose={props.onClose}><button onClick={props.onToggle} className={`flex w-full items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-xs font-medium ${props.active ? "bg-rose-50 text-rose-700" : "bg-slate-950 text-white"}`}>{props.active ? <CircleStop size={15} /> : <Play size={15} />}{props.active ? t("action.stopRecording") : t("action.startRecording")}</button><ol className="mt-3 max-h-40 space-y-1.5 overflow-auto">{props.actions.map((action) => <li key={action.id} className="rounded-lg bg-slate-50 px-2.5 py-2 text-[10px] text-slate-600">{t(`browserAction.${action.action}`)} · {action.label || action.selector || t("recording.page")}</li>)}</ol><label className="mt-3 block text-[10px] font-medium text-slate-500">{t("recording.skillName")}<input value={props.name} onChange={(event) => props.onName(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-xs outline-none focus:border-violet-300" /></label><label className="mt-3 block text-[10px] font-medium text-slate-500">{t("recording.description")}<textarea value={props.description} onChange={(event) => props.onDescription(event.target.value)} rows={2} className="mt-1 w-full resize-none rounded-xl border border-slate-200 px-3 py-2 text-xs outline-none focus:border-violet-300" /></label><div className="mt-4 flex justify-end gap-2"><button onClick={props.onReplay} disabled={!props.actions.length} className="rounded-xl px-3 py-2 text-xs text-slate-500 disabled:opacity-40">{t("action.testReplay")}</button><button onClick={props.onSave} disabled={!props.actions.length || props.active} className="rounded-xl bg-slate-950 px-3 py-2 text-xs font-medium text-white disabled:bg-slate-200">{props.editing ? t("action.updateSkill") : t("action.saveSkill")}</button></div></ModalShell>;
}
