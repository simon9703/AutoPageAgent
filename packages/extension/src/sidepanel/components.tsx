...
export type SkillView = "mine" | "marketplace";
...
export function SkillsModal(props: { view: SkillView; setView: (view: SkillView) => void; items: Array<PageSkillSummary | SkillCatalogItem>; selectedSlug: string; onClose: () => void; onRefresh: () => void; onAdd: () => void; onImport: (bundle: SkillExportBundle) => void; onUse: (skill: Pick<SkillCatalogItem, "name" | "description" | "slug">, debug?: boolean) => void; onInstall: (slug: string, update: boolean) => void; onToggle: (slug: string, enabled: boolean) => void; onEdit: (slug: string) => void; onDelete: (slug: string, name: string) => void; onExport: (slug: string) => void }) {
  const { t } = useTranslation();
  const importRef = useRef<HTMLInputElement>(null);
  return <ModalShell title={t("skills.title")} subtitle={t("skills.subtitle")} onClose={props.onClose} action={<><IconButton label={t("action.addSkill")} onClick={props.onAdd}><Plus size={15} /></IconButton><IconButton label={t("action.importSkill")} onClick={() => importRef.current?.click()}><Upload size={15} /></IconButton><IconButton label={t("skills.refresh")} onClick={props.onRefresh}><RefreshCw size={15} /></IconButton><input ref={importRef} type="file" accept=".json,application/json" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; event.currentTarget.value = ""; if (!file) return; void file.text().then((text) => { try { props.onImport(JSON.parse(text) as SkillExportBundle); } catch { alert(t("notice.invalidSkillFile")); } }); }} /></>}><div className="grid grid-cols-2 rounded-xl bg-slate-100 p-1">{(["mine", "marketplace"] as const).map((view) => <button key={view} onClick={() => props.setView(view)} className={`rounded-lg px-2 py-2 text-[11px] font-medium ${props.view === view ? "bg-white text-slate-900 shadow-sm" : "text-slate-500"}`}>{view === "mine" ? t("skills.mine") : t("skills.market")}</button>)}</div><p className="px-1 pt-3 text-[10px] text-slate-400">{t("skills.count", { count: props.items.length })}</p><div className="mt-2 space-y-2">{props.items.length ? props.items.map((skill) => <SkillRow key={skill.slug} skill={skill} view={props.view} selected={props.selectedSlug === skill.slug} onUse={props.onUse} onInstall={props.onInstall} onToggle={props.onToggle} onEdit={props.onEdit} onDelete={props.onDelete} onExport={props.onExport} />) : <p className="rounded-2xl border border-dashed border-slate-200 px-4 py-8 text-center text-xs text-slate-400">{t("skills.empty")}</p>}</div></ModalShell>;
}
