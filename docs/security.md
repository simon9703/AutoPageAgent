...
- `dismiss` is not an arbitrary click escape hatch. It accepts only a latest-snapshot ref for an expanded combobox, visible listbox/menu, selected option belonging to the popup, or the topmost dialog. A selected option only anchors the popup and is not clicked. For inner popups, the content script sends Escape, checks the live state, then computes and revalidates one non-interactive point outside the closest visible popup boundary when needed. Only during an active top-frame Agent run, the background briefly attaches Chrome Debugger Protocol, dispatches one trusted `mouseMoved -> mousePressed -> mouseReleased` sequence at that internally computed point, and immediately detaches. The model, page, side panel, and bridge never supply or receive the coordinate. Modal dropdowns remain restricted to Modal content so the outer dialog is preserved. The whole dismiss is one attempt and one possible failure. Success still requires a fresh snapshot proving `aria-expanded=false` or controlled-popup disappearance.
- After a verified option click, the runtime may insert this same constrained `dismiss` before moving to another field or asking the provider to continue. It does not insert it while another unselected option from the same popup is queued, and it never converts the housekeeping step into an unrestricted coordinate action.
...
- The `debugger` permission is used only for this bounded trusted exterior click. Requests are rejected unless they originate from the active Agent run's top-frame content script and target the same tab; no general CDP method or coordinate action is exposed through the Agent protocol.
