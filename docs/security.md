...
- `dismiss` is not an arbitrary keyboard or click escape hatch. It accepts only a latest-snapshot ref for an expanded combobox, visible listbox/menu, selected option belonging to the popup, or the topmost dialog. A selected option only anchors the popup and is not clicked. For inner popups, the content script sends a synthetic Escape and checks the live state. If the popup remains open, the background briefly attaches Chrome Debugger Protocol, dispatches one extension-fixed trusted `rawKeyDown(Escape) -> keyUp(Escape)` sequence, immediately detaches, and checks the live state again. Only then may it compute and revalidate non-interactive points outside the closest visible popup boundary and request bounded trusted `mouseMoved -> mousePressed -> mouseReleased` clicks. The model, page, side panel, and bridge never supply a key, coordinate, or selector. Modal dropdowns remain restricted to Modal content so the outer dialog is preserved. The whole sequence is one dismiss attempt and one possible failure. Success still requires a fresh snapshot proving `aria-expanded=false` or controlled-popup disappearance.
