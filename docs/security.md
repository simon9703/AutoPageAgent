...
- `dismiss` is not an arbitrary click escape hatch. It accepts only a latest-snapshot ref for an expanded combobox, visible listbox/menu, selected option belonging to the popup, or the topmost dialog. A selected option only anchors the popup and is not clicked. For inner popups, the content script sends Escape, checks the live state, then computes one verified non-interactive point outside the closest visible popup boundary when needed. Modal dropdowns remain restricted to Modal content so the outer dialog is preserved. The whole dismiss is one attempt and one possible failure. Success still requires a fresh snapshot proving `aria-expanded=false` or controlled-popup disappearance.
