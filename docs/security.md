...
- `dismiss` is not an arbitrary click escape hatch. It accepts only a latest-snapshot ref for an expanded combobox, visible listbox/menu, or the topmost dialog. For inner popups, the content script clicks one verified non-interactive point outside the measured visible popup boundary and uses Escape only when no safe exterior point exists. Modal dropdowns remain restricted to Modal content so the outer dialog is preserved. The whole dismiss is one attempt and one possible failure. Success still requires a fresh snapshot proving `aria-expanded=false` or controlled-popup disappearance.
