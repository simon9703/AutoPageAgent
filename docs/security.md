...
- Editable, non-sensitive input/textarea comboboxes may receive a bounded `fill` only to filter options. Readonly comboboxes remain non-writable; a combobox filter fill must end the plan, and the filtered option can be clicked only through a ref from the following fresh Snapshot.
