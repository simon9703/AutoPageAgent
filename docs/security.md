...
- `dismiss` is not an arbitrary click escape hatch. It accepts only a latest-snapshot ref for an expanded combobox, visible listbox/menu, or the topmost dialog. The runtime focuses the exact control or ARIA popup owner before sending complete Escape keydown/keyup events; if closure is still unverified, it may blur only that resolved combobox. Success still requires a fresh snapshot proving `aria-expanded=false` or controlled-popup disappearance. An inner popup must be closed before an outer dialog, and dialogs containing filled state are dismissed only when the original user task explicitly asks to cancel or close that dialog.
