...
Direct state actions such as ordinary fill and focus use a short wait; a searchable combobox fill polls the existing mutation-backed DOM state for a visible option for up to 1.2 seconds and then waits for a short quiet period before the fresh snapshot. Because a dynamic option does not exist in the original snapshot, it is not queued in advance: the provider plans `fill`, receives a fresh observation, then plans the visible option click. When click or submit has no verified effect in its first observation, the content script performs one mutation-aware delayed observation for at most 2.5 seconds; other actions do not inherit this extra wait. Dynamic options use lightweight ARIA selectors and ephemeral refs; zero-size, covered, disabled, and readonly candidates remain excluded. Native `select` remains the only target accepted by the `select` action. Buttons and button-like controls always use `click`, regardless of labels such as Submit, Pay, Confirm, or Top Up; provider output that incorrectly assigns `submit` to a non-form target is normalized to `click`, and the content runtime keeps the same fallback.
