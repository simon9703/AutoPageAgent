...
The side panel can attach one inspected element, page image, or captured viewport to the next message. Both the Responses provider and Local Codex send the selected visual as an image input; Local Codex materializes data URLs in a private temporary directory for one turn and deletes them immediately afterward. Screenshot data URLs are always removed from the text prompt. A successful initial agent response consumes the pending context and clears it from the composer. The user message keeps a compact, read-only attachment summary across side-panel reloads, while later agent-history requests omit the summary and all screenshot binary data.

Visual analysis is adaptive, not a second per-step observation stream. The content snapshot records only cheap counts for visible images, large images, canvases, and videos. An obvious visual surface can request an initial screenshot; otherwise the agent stays DOM-only. A blocked decision first receives the normal bounded semantic-change and quiet-DOM recovery. Only when that recovery produces no actionable state may the same initial or action boundary capture one active-viewport screenshot and replan once with the current DOM. Automatic capture never activates another tab and is skipped when the snapshot exposes sensitive fields. When the internal `BACKGROUND_FEATURE_FLAGS.automaticScreenshotVisualMarks` flag is enabled, automatic visual screenshots draw bounded red numbered outlines after capture, without modifying the page DOM; disabling it preserves the existing clean automatic screenshot path. The flag is source-controlled in the extension background and intentionally has no page or side-panel setting. Each number uses the same global `[N]` index shown in `simplifiedDom` and carries an ephemeral `index -> ref` map for that exact snapshot. The content script validates the snapshot id, URL, and DOM version both before and after capture; a changed page discards the visual rather than attaching stale marks. The model must still return the mapped current DOM ref; numbers and coordinates are never executable action targets. The screenshot may explain a visual state, but actions still require current DOM refs and task completion still requires exact DOM text or URL evidence.
...
Multi-select popups use the constrained `dismiss` action after all required unselected options have been chosen. After an option click is verified, the background queue preserves any next unselected option belonging to the same popup. Otherwise, while that popup remains visible, it inserts a trusted `dismiss` step before another field or provider continuation; this prevents a model retry from clicking the combobox input again. Its target must be a latest-snapshot ref for an expanded combobox, visible listbox/menu, a selected option that belongs to the open popup, or the topmost dialog. A selected option is only a safe popup anchor and is never clicked again. For popup targets, the content runtime sends Escape first, checks the live state, then resolves the closest visible positioned popup boundary—including a visible ancestor when the ARIA listbox itself is zero-sized—and selects one verified non-interactive exterior DOM element. Inside a Modal, the search stays in Modal content outside the dropdown; otherwise it stays in the viewport outside the popup. Buttons, links, inputs, focusable controls, pointer cursors, and the agent overlay are rejected, so the previous input cannot become the fallback. The AI pointer moves to the exact resolved coordinate. The content script revalidates the point immediately before activation and asks the background for one bounded trusted click. The background accepts that request only from the active Agent run's top frame and same tab, briefly attaches CDP, dispatches `Input.dispatchMouseEvent` as `mouseMoved -> mousePressed -> mouseReleased`, and immediately detaches. This produces the same trusted browser input as a physical mouse instead of an `isTrusted=false` DOM event. The runtime then checks the live expanded state and popup visibility, captures one fresh snapshot, and verifies an exact `aria-expanded: true -> false` transition or controlled-popup disappearance; all internal strategies produce only one verification and at most one failure. Existing outer dialogs must remain present when an inner popup is dismissed; an open dropdown blocks dialog dismissal. A filled dialog requires a bridge-attached trusted authorization derived from an explicit cancel/close request in the original user task; provider-authored flags are ignored. A snapshot-visible close button or backdrop continues to use ordinary `click`; model-authored coordinates, selectors, XPath, CSS-framework class names, parent-trigger lookup, repeated combobox clicking, and blind ancestor clicking are never accepted.
